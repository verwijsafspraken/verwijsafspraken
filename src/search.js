...
const { SearchModal } = require("./modals.js");
const { renderSearchModal, renderSearchResults, stringifyHtml } = require("./rendering.js");
let fuse;
...
function flattenArticles(article) {
  return [article, ...(article.children ? article.children.flatMap(flattenArticles) : [])];
}
...
function anyOfRegExp(texts) {
  return new RegExp(texts.map((text) => (text instanceof RegExp ? text.source : escapeRegExp(text))).join("|"), "giu");
}

const markdownRegExp = anyOfRegExp([/^\* /, /^#+ /, "*", "**", "_", ">", "`"]);
...
function initSearch(database) {
  ...
  for (const [articleIndex, article] of articles.entries()) {
    const text = demarkdown([article.name, article.blurb, article.content].filter(Boolean).join("\n\n"));
...
function createArticleMatches(query, articles) {
  ...
  return articles.map((article) => {
    const matches = keys.map((key) => {
      ...
      const indices = [...value.matchAll(wordRegexp)].map((match) => [match.index, match.index + match[0].length - 1]);
...
function openSearch() {
  ...
  newInput.addEventListener("input", (event) => {
    ...
    resultList.innerHTML = stringifyHtml(renderSearchResults(search(query)));
  });
  newInput.addEventListener("keydown", (event) => {
    if (event.key === "Enter") (resultList.querySelector("a:hover") || resultList.querySelector("a"))?.click();
  });
  newInput.focus();
}
